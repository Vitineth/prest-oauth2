 ...
 * <br/><br/>
 * This file shows a basic example of how to create an OAuth2 web server using nodes HTTP library. This uses three
 * addresses as listed below.
 * <br/><br/>
 * This version stores it in memory only as a basic example of how to use the controller. This has been verified as
 * working wen using the requests_oauthlib python library with the code listed here:
 * https://gist.github.com/Vitineth/33ade186d8dbf4868de851282db2b6a6 The code should be reasonably simple but you will
 * need to change a few aspects in order to make sure that the code actually works such as any addresses or ports that
 * you decide to change in this example.
 */

// [1] - Step One - Define a user model. Something that you will use to identify the users associated with the requests
...
// [2] - Step Two - Set up some storage medium that you will be able to interact with
...
let accessTokens: { [index: string]: OAuth2AccessToken } = {};
let authorizationCodes: { [index: string]: OAuth2AuthorizationCode } = {};
let refreshTokens: { [index: string]: OAuth2RefreshToken } = {};
let clients: { [index: string]: OAuth2Client } = {};
...
//Define a key and a secret
let clientID = "809EO04Eo4dOPS5CTq8Ct6gEeX1bw3LPaaPMzA1B";
let clientSecret = "Wn9A886pLBzzVyJX8DybdoDNp2ynu053d50V051K";
//I've taken a few liberties here, you will want to define some proper scopes for your client as well as a real redirect uri.
clients[clientID] = new OAuth2Client(null, "dustem.js", clientID, clientSecret, "https://vitineth.me/oauth2/dustem", "any", ["any"], 1, users[1]);

// [3] - Step Three - Define the controller
 ...
 */
class MemoryOnlyController implements OAuth2Controller {
    ...
    generateRawAuthorizationCode(): string {
        return MemoryOnlyController.generateCode();
    }
    ...
    fetchAccessToken(token: string): Promise<OAuth2AccessToken | boolean> {
        return Promise.resolve(accessTokens.hasOwnProperty(token) ? accessTokens[token] : false);
    }

    fetchRefreshToken(token: string): Promise<OAuth2RefreshToken | boolean> {
        return Promise.resolve(refreshTokens.hasOwnProperty(token) ? refreshTokens[token] : false);
    }

    fetchAuthorizationCode(code: string): Promise<OAuth2AuthorizationCode | boolean> {
        return Promise.resolve(authorizationCodes.hasOwnProperty(code) ? authorizationCodes[code] : false);
    }

    fetchClient(id: string): Promise<OAuth2Client | boolean> {
        return Promise.resolve(clients.hasOwnProperty(id) ? clients[id] : false);
    }
    ...
    saveAccessToken(token: OAuth2AccessToken): boolean {
        accessTokens[token.code] = token;
        return true;
    }

    saveRefreshToken(token: OAuth2RefreshToken): boolean {
        refreshTokens[token.code] = token;
        return true;
    }
    ...
    revokeAccessToken(token: OAuth2AccessToken): boolean {
        if (accessTokens.hasOwnProperty(token.code)) {
            delete accessTokens[token.code];
            return true;
        }
        return false;
    }
    ...
    revokeRefreshToken(token: OAuth2RefreshToken): boolean {
        if (refreshTokens.hasOwnProperty(token.code)) {
            delete refreshTokens[token.code];
            return true;
        }
        return false;
    }
    ...
    revokeAuthorizationCode(code: OAuth2AuthorizationCode): boolean {
        if (authorizationCodes.hasOwnProperty(code.code)) {
 ...
 */
let controller = new MemoryOnlyController();
...
// [4a] - Step Four A - Define the request handlers that will server the various requests.
        ...
        client.then((client) => {
            ...
            authorization.then((authorization) => {
                ...
                handler.token(grantTypes, clientID, clientSecret, code, scopes, redirectURI).then((token) => {
                    ...
                    response.statusCode = 400;
                    response.end(JSON.stringify({"error": "Failed to authorize the Failed to activate the token", "reason": reason}));
                });
            }).catch((reason) => {
                response.statusCode = 400;
                response.end(JSON.stringify({"error": "Failed to fetch the authorization code. This is most likely due to an invalid code", "reason": reason}));
            })
        }).catch((reason) => {
            response.statusCode = 400;
            response.end(JSON.stringify({"error": "Failed to fetch the client. The client ID is most likely invalid", "reason": reason}));
        })
    }).catch(() => {
...
// [4b] - Step Four B - Set up and start the server
